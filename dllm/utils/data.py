...
def post_process_dataset(
    ...
    """
    # Always filter out empty samples first
    dataset = dataset.filter(
        lambda row: len(row["input_ids"]) > 0,
    ...
    if data_args.truncation == "filter":
        return dataset.filter(
            ...
            num_proc=data_args.num_proc,
            desc=f"Filtering samples with 0 < length <= {data_args.max_length}",
        )
...
def default_mdlm_sft_map_fn(row, *, tokenizer, mask_prompt_loss: bool = True) -> dict:
    ...
    """
    # Skip rows with None content in messages
    for msg in row["messages"]:
        if msg.get("content") is None:
            return {"input_ids": [], "labels": [], "prompt_len": 0}

    # enable_thinking=False is Qwen3-specific; skip for other models
    extra_kwargs = {}
