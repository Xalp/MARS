...
class BD3LMTrainer(MDLMTrainer):
    ...
    def compute_loss(
        ...
        **kwargs,
        ...
        if mismatch.any():
            # Correct input_ids to match labels for consistency in context/loss
            # Note: We need to use indices that map back to the full tensor
            # masked_indices is boolean mask of shape [B, L]
            # We enforce input_ids = labels at these positions
