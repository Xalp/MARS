"""
Full Mask Sampler for CSBD3LMv2 models trained with 100% masking.

The key difference from standard BD3LM/CSBD3LM samplers:
- Standard: Tokens are revealed based on confidence (most confident first).
- FullMask: Tokens are revealed LEFT-TO-RIGHT within the block.
...
from dllm.core.samplers.bd3lm import BD3LMSamplerConfig, BD3LMSampler
from dllm.core.samplers.csbd3lm import build_csbd_attention_mask
...
def build_pure_causal_attention_mask(
    ...
    causal_mask = pos.view(1, T, 1) >= pos.view(1, 1, T) # [1, T, T] broadcasable
...
@dataclass
class FullMaskSamplerConfig(BD3LMSamplerConfig):
    """Config for FullMaskSampler."""
...
@dataclass
class FullMaskSampler(BD3LMSampler):
    """
    Sampler for models trained with CSBD3LMv2 Full Masking.

    Uses left-to-right token acceptance within each block.
    Matches CSBD3LMv2Sampler cache pattern for efficiency.
    """
    ...
    def get_global_stats(self) -> dict:
        ...
        if fp > 0 and is_main:
            print(f"[FullMask Global Stats] {stats}")
        return stats
    ...
    def sample(
        ...
        config: FullMaskSamplerConfig | None = None,
        **kwargs,
    ) -> SamplerOutput | torch.Tensor:
        if config is None:
            config = FullMaskSamplerConfig()
