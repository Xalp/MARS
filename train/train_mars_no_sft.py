"""
Stage 2: MARS Training (without SFT loss).
Trains the model to predict masked blocks without auxiliary AR loss on clean stream.
...
Usage:
    accelerate launch --config_file configs/zero2.yaml --num_processes 8 \
        ...
        --output_dir models/mars_blk4_no_sft \
...
from mars.trainers import MARSTrainerNoSFT
...
class TrainingArguments(dllm.utils.TrainingArguments):
    output_dir: str = "models/mars_blk4_no_sft"
...
def train():
    ...
    logger.info("Start MARS training (without SFT loss)...")
    trainer = MARSTrainerNoSFT(
        model=model,
        tokenizer=tokenizer,
